...
use crate::engine::wire::{
    serialize_settlement_with_root, serialize_state_update, Settlement, StateUpdate,
};
...
impl Endpoint {
    ...
    fn sign(&self, msg: &[u8]) -> [u8; 64] {
        self.signing
            .as_ref()
            .expect("controlled endpoint must sign")
            .sign(msg)
    }
...
impl DistTunnel {
    ...
    pub fn is_terminal(&self) -> bool {
        crate::game::blackjack::is_terminal(&self.state)
    }
    ...
    fn on_move(&mut self, frame: MoveFrame) -> Vec<Vec<u8>> {
        ...
        assert!(
            next.balance_a == frame.party_a_balance && next.balance_b == frame.party_b_balance,
            "frame balances mismatch"
        );
        ...
        let msg = serialize_state_update(&update);
        assert!(
            self.opp_ep.verify(&msg, &frame.sig_proposer),
            "proposer signature failed"
        );
        let sig_responder = self.self_ep.sign(&msg);
        self.state = next;
        self.nonce = frame.nonce;
        vec![encode_frame(&Frame::Ack(AckFrame {
            nonce: frame.nonce,
            sig_responder,
        }))]
    }
    ...
    fn on_ack(&mut self, frame: AckFrame) -> Vec<Vec<u8>> {
        ...
        assert!(
            self.opp_ep.verify(&p.msg, &frame.sig_responder),
            "responder signature failed"
        );
    ...
    pub fn build_settlement_half_with_root(
        &self,
        timestamp: u64,
        root: &[u8; 32],
        onchain_nonce: u64,
    ...
    pub fn combine_settlement_with_root(
        &self,
        settlement: &Settlement,
        root: &[u8; 32],
        sig_self: &[u8; 64],
        sig_other: &[u8; 64],
    ) -> Result<([u8; 64], [u8; 64]), String> {
...
mod tests {
    ...
    fn seats() -> (DistTunnel, DistTunnel) {
        ...
        let a = DistTunnel::new(
            "0xab",
            Party::A,
            Endpoint::controlled(&sa),
