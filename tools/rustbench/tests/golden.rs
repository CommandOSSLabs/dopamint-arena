...
fn engine_reproduces_all_golden_vectors() {
    ...
    let settle_v2 = serialize_settlement_with_root(
        &Settlement {
            tunnel_id: "0xab".into(),
            party_a_balance: 1000,
            party_b_balance: 2000,
    ...
    assert_eq!(
        hex::encode(compute_commitment(&[7], &salt_a).unwrap()),
        field(json, "commitment")
    );
    assert_eq!(
        hex::encode(combine_reveals(&[7], &salt_a, &[42], &salt_b)),
        field(json, "seed")
    );
    assert_eq!(
        hex::encode(blake2b256(b"hello")),
        field(json, "blake2b_hello")
    );
