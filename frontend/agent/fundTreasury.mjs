...
if (!process.env.SUI_TREASURY_KEY) {
  throw new Error(
    "set SUI_TREASURY_KEY (suiprivkey1… via `sui keytool export`)",
  );
}

const keysPath = new URL("./keys.json", import.meta.url);
const existing = existsSync(keysPath)
  ? JSON.parse(readFileSync(keysPath, "utf8"))
  : [];
const want = N - existing.length;
if (want <= 0) {
  console.log(
    `already have ${existing.length} agent wallet(s) >= ${N}; reusing keys.json, no funding`,
  );
  process.exit(0);
}
...
console.log(
  `funded ${want} new agent wallets (total ${N}) | digest ${res.digest}`,
);
