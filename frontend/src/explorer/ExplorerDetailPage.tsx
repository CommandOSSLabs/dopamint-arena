...
import { suivisionTxUrl, suivisionAccountUrl, truncateMiddle } from "@/lib/suivision";
...
export function ExplorerDetailPage() {
  ...
  useEffect(() => {
    getSettlement(digest).then(setRow).catch((e) => setError(String(e)));
  }, [digest]);
          ...
          {row && (
            <>
              <Field label="Settle tx">
                <a href={suivisionTxUrl(row.txDigest, network)} target="_blank" rel="noreferrer noopener" className="hover:text-primary hover:underline">
              ...
              <Field label="Party A">
                {row.partyAAddr ? (
                  <a href={suivisionAccountUrl(row.partyAAddr, network)} target="_blank" rel="noreferrer noopener" className="hover:text-primary hover:underline">
              ...
              <Field label="Party B">
                {row.partyBAddr ? (
                  <a href={suivisionAccountUrl(row.partyBAddr, network)} target="_blank" rel="noreferrer noopener" className="hover:text-primary hover:underline">
