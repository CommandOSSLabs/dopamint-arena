...
/** Console-debug a scoped event (no-op when debug is off). */
export function elog(scope: string, ...args: unknown[]): void {
  if (ENGINE_DEBUG) console.debug(`[engine:${scope}]`, ...args);
}
...
export function emark(scope: string, label: string): () => void {
  ...
  return () =>
    console.debug(`[engine:${scope}] ${label} — ${(performance.now() - t0).toFixed(1)}ms`);
}
