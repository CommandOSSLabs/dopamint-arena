...
import type { MatchChannel, SettlementSigner } from "./seams";
...
describe("PvpGameSession cooperative close", () => {
  ...
  it("settle path: phase -> 'done', only seat A calls submitCooperativeClose", async () => {
    ...
    const { signer: signerA, submitCallCount: countA } =
      makeFakeSettlementSigner("ok");
    const { signer: signerB, submitCallCount: countB } =
      makeFakeSettlementSigner("ok");

    const settleA = sA.settle({
      channel: chA,
      settlementSigner: signerA,
      createdAt: 0n,
      onchainNonce: 0n,
    });
    const settleB = sB.settle({
      channel: chB,
    ...
    assert.strictEqual(
      countA(),
      1,
      "seat A must call submitCooperativeClose exactly once",
    );
    assert.strictEqual(
      countB(),
      0,
      "seat B must NOT call submitCooperativeClose",
    );
  ...
  it("root mismatch -> phase 'error', no submit", async () => {
    ...
    let aSettleHalfCb: ((h: { sig: string; root: string }) => void) | null =
      null;
    ...
    const realChA = chA as FakeSettleChannel & {
      _deliverSettle(h: { sig: string; root: string }): void;
    };
    ...
    assert.strictEqual(
      ...
      "error",
      "root mismatch must set phase error",
    );
  ...
  it("hard submit failure routes seat A to 'error'", async () => {
    ...
    assert.strictEqual(
      sA.getSnapshot().phase,
      "error",
      "hard submit failure must set error phase",
    );
  });
});
