...
describe("tunnel state helpers", () => {
  ...
  it("settlement message (1/1) verifies for both parties", () => {
    ...
    expect(core.verify(core.sign(msg, kpA.secretKey), msg, kpA.publicKey)).toBe(
      true,
    );
    expect(core.verify(core.sign(msg, kpB.secretKey), msg, kpB.publicKey)).toBe(
      true,
    );
  });
  ...
  it("selfPlay plays a full game and the settlement sums to the locked total", () => {
    ...
    const t = core.OffchainTunnel.selfPlay(proto, TID, kpA, kpB, "0xa", "0xb", {
      a: 1n,
      b: 1n,
    });
