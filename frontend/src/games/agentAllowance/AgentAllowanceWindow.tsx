  ...
  return (
    <Shell>
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <CtrlButton onClick={onRevoke} disabled={Boolean(busy)} icon={CircleSlash} danger>
          Stop
        </CtrlButton>
      </div>
      ...
      <ul className="flex flex-col gap-1">
        ...
        {ledger.map((e, i) => (
          <li
            key={`${e.digest}-${i}`}
            className="flex items-center justify-between rounded border border-arena-edge bg-arena-bg/40 px-2 py-1 text-xs"
          >
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="font-mono text-[11px] text-arena-accent hover:underline"
            >
              {shortAddr(e.digest)}
            </a>
          </li>
