...
function buildCommandOss(): StrokeTemplate {
  ...
  const paths: TemplatePath[] = [
    ...
    { kind: "fill", color: SUI_BLUE, rings: [regularPolygon(cx, cy, 14, 8, Math.PI / 8)] },
    // White "</>" open-source code glyph over the badge.
    { kind: "stroke", color: WHITE, radius: 1.0, points: [{ x: 11, y: 9.5 }, { x: 6.5, y: 15 }, { x: 11, y: 20.5 }] },
    { kind: "stroke", color: WHITE, radius: 1.0, points: [{ x: 18, y: 8.5 }, { x: 12, y: 21.5 }] },
    { kind: "stroke", color: WHITE, radius: 1.0, points: [{ x: 19, y: 9.5 }, { x: 23.5, y: 15 }, { x: 19, y: 20.5 }] },
  ];
  return { id: "commandoss", name: "CommandOSS", category: "logo", aspect: { w: 30, h: 30 }, paths };
}
...
function buildVnFlag(): StrokeTemplate {
  ...
  return {
    ...
    paths: [
      { kind: "fill", color: RED, rings: [[{ x: 0, y: 0 }, { x: w, y: 0 }, { x: w, y: h }, { x: 0, y: h }]] },
      { kind: "fill", color: YELLOW, rings: [starPolygon(w / 2, h / 2, 8.4, 3.5)] },
    ],
  };
}
...
function buildVnStar(): StrokeTemplate {
  return {
    ...
    category: "vietnam",
    aspect: { w: 20, h: 20 },
    paths: [{ kind: "fill", color: YELLOW, rings: [starPolygon(10, 10, 9, 3.75)] }],
  };
}

// ── Lotus — layered pink petals, gold center, green stem ──────────────────────────
function lotusPetal(cx: number, cy: number, len: number, halfW: number, angle: number): Vec2[] {
  const ring: Vec2[] = [];
...
function buildLotus(): StrokeTemplate {
  ...
  paths.push({
    ...
    color: GREEN,
    radius: 0.8,
    points: [{ x: 15, y: 25 }, { x: 14.5, y: 29 }, { x: 15, y: 33 }],
  });
  paths.push({ kind: "fill", color: LIGHT_GREEN, rings: [lotusPetal(13, 28, 6, 2, Math.PI * 0.92)] });
  paths.push({ kind: "fill", color: LIGHT_GREEN, rings: [lotusPetal(17, 30, 6, 2, Math.PI * 0.08)] });
  // Outer ring of 8 petals, then an inner offset ring of 8 — a layered bloom.
  for (let k = 0; k < 8; k++) {
    const a = (k / 8) * Math.PI * 2 - Math.PI / 2;
    paths.push({ kind: "fill", color: PINK, rings: [lotusPetal(cx, cy, 11, 3.1, a)] });
  }
  for (let k = 0; k < 8; k++) {
    const a = (k / 8) * Math.PI * 2 - Math.PI / 2 + Math.PI / 8;
    paths.push({ kind: "fill", color: RED, rings: [lotusPetal(cx, cy, 7.5, 2.3, a)] });
  }
  paths.push({ kind: "fill", color: YELLOW, rings: [circle(cx, cy, 2.6, 14)] });
  return { id: "lotus", name: "Lotus", category: "vietnam", aspect: { w: 30, h: 34 }, paths };
}

// ── Đông Hồ pig — brown line-art with the signature flank spiral ───────────────────
function spiralPoints(cx: number, cy: number, turns: number, maxR: number, step = 0.3): Vec2[] {
  const pts: Vec2[] = [];
...
function buildDongHo(): StrokeTemplate {
  ...
  const paths: TemplatePath[] = [
    ...
    { kind: "stroke", color: ink, radius: 0.85, closed: true, points: ovalRing(19, 13, 15, 8) },
    // Ear.
    { kind: "stroke", color: ink, radius: 0.7, closed: true, points: [{ x: 7, y: 7 }, { x: 10, y: 2.5 }, { x: 12.5, y: 7.5 }] },
    // Snout (small ring at the front).
    { kind: "stroke", color: ink, radius: 0.7, closed: true, points: circle(4.2, 13.5, 2.2, 12) },
    // Eye (a filled dab).
    { kind: "stroke", color: ink, radius: 1.0, points: [{ x: 9.5, y: 10.5 }] },
    // Four legs.
    { kind: "stroke", color: ink, radius: 0.7, points: [{ x: 9, y: 19.5 }, { x: 8.5, y: 26 }] },
    { kind: "stroke", color: ink, radius: 0.7, points: [{ x: 15, y: 20.5 }, { x: 15, y: 27 }] },
    { kind: "stroke", color: ink, radius: 0.7, points: [{ x: 24, y: 20.5 }, { x: 24.5, y: 27 }] },
    { kind: "stroke", color: ink, radius: 0.7, points: [{ x: 30, y: 19.5 }, { x: 31, y: 26 }] },
    // Curly tail at the back.
    { kind: "stroke", color: ink, radius: 0.6, points: [{ x: 34, y: 11 }, { x: 37, y: 9 }, { x: 37, y: 13 }, { x: 34.5, y: 13 }] },
    // The iconic âm-dương flank spiral.
    { kind: "stroke", color: ink, radius: 0.6, points: spiralPoints(22, 13, 1.6, 5.5) },
  ];
  return { id: "dong-ho", name: "Đông Hồ Pig", category: "vietnam", aspect: { w: 38, h: 28 }, paths };
}

/** A closed ellipse ring (used for the pig body). */
function ovalRing(cx: number, cy: number, rx: number, ry: number, segs = 22): Vec2[] {
  const pts: Vec2[] = [];
...
function buildStar(): StrokeTemplate {
  return {
    ...
    paths: [
      ...
      { kind: "stroke", color: ORANGE, radius: 0.6, closed: true, points: starPolygon(11, 11, 10.2, 4.2) },
    ],
  };
}
...
export const TEMPLATES_BY_ID: Record<string, StrokeTemplate> = Object.fromEntries(
  TEMPLATES.map((t) => [t.id, t]),
);

/** Category render order for the grouped pickers. */
export const TEMPLATE_CATEGORIES = ["logo", "vietnam", "shape", "text"] as const;
