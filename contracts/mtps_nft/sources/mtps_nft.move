...
///
/// Minting is free, but fields are validated — non-empty title and image, bounded lengths — so it
...
module mtps_nft::mtps_nft {
    ...
    /// Permissionless and free: anyone mints a `(title, description, image_url)` NFT to THEMSELVES.
    /// UTF-8 bytes in; transferred to the caller. Aborts on an empty title/image or an over-length
    /// field.
