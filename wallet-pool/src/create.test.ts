...
test("create writes a sealed pool with N members + master", async () => {
  ...
  const res = await create({ network: "testnet", members: 3, master: { generate: true }, store });
  ...
  const members = JSON.parse(new TextDecoder().decode(unseal(blob.crypto, res.accessValue, aadFor(blob)))) as SealedMembers;
...
test("imported master seed lands at correct address", async () => {
  ...
  const expected = Ed25519Keypair.fromSecretKey(seed).getPublicKey().toSuiAddress();
  const store = memStore();
  const res = await create({ network: "testnet", members: 1, master: { seed }, store });
  const blob = parseBlob((await store.read(res.walletPoolId))!);
