...
import { serializeBlob, parseBlob, aadFor, encodeMembers, decodeMemberSecret } from "./blob";
...
const sample: PoolBlob = {
  ...
  index: [{ role: "master", address: "0x1", ordinal: 0, createdAt: 0, enabled: true, useCount: 0, lastUsedAt: 0 }],
};
...
test("aadFor is deterministic and identity-bound", () => {
  ...
  assert.notDeepEqual(aadFor(sample), aadFor({ ...sample, network: "testnet" }));
});

test("members encode/decode round-trips", () => {
  const kp = generateKeyPair();
  const enc = encodeMembers(kp.secretKey, [{ ordinal: 1, secret: kp.secretKey }]);
