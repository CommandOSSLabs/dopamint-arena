    ...
    async fn plan_move(
        ...
        match state.phase {
            ...
            | PokerPhase::RiverBet => {
                ...
                // Effective stack (mirrors the protocol's bet/call gate) — betting past
                // what the short stack can cover is rejected, so clamp to the same bound.
...
fn estimate_strength_profile(
    ...
    // Effective stack (min of both balances less committed), matching `available_for`
    // so the persona never plans a bet/call the protocol would reject.
    let available = available_for(state, seat);
