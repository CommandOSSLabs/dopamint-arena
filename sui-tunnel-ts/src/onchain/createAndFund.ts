...
/**
 * Append one `create_and_fund` call, funding both parties from two caller-supplied coins. Targets
 * the function present on the deployed package; the funder need not be a party. The new tunnel's
 * id is read from tx effects (the deployed `create_and_fund` returns nothing).
 */
