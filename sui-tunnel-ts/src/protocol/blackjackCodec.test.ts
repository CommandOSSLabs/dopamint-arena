...
test("commit round-trips its 32-byte commitment and DROPS localSecret from the wire", () => {
  ...
  const localSecret = {
    value: Uint8Array.from([9]),
    salt: new Uint8Array(16).fill(3),
  };
  const json = blackjackMoveCodec.encode({
    kind: "commit",
    commitment,
    localSecret,
  });
  ...
  const out = blackjackMoveCodec.decode(json) as Extract<
    BlackjackMove,
    { kind: "commit" }
  >;
...
test("reveal round-trips value and salt bytes", () => {
  ...
  const out = rt({ kind: "reveal", reveal }) as Extract<
    BlackjackMove,
    { kind: "reveal" }
  >;
...
test("an unknown move kind is rejected on decode", () => {
  assert.throws(
    () => blackjackMoveCodec.decode({ kind: "nope" }),
    /unsupported/,
  );
});

test("JSON survives a stringify/parse trip (real relay path)", () => {
  const m: BlackjackMove = {
    kind: "reveal",
    reveal: { value: Uint8Array.from([4]), salt: new Uint8Array(16).fill(2) },
  };
  const wire = JSON.parse(JSON.stringify(blackjackMoveCodec.encode(m)));
  const out = blackjackMoveCodec.decode(wire) as Extract<
    BlackjackMove,
    { kind: "reveal" }
  >;
  assert.deepEqual(
    out.reveal.salt,
    (m as Extract<BlackjackMove, { kind: "reveal" }>).reveal.salt,
  );
});
