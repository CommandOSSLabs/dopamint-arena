# 0028 — Arena open: bot pre-creates + funds seat B at allocate; user joins deposit-only

- **Status**: Proposed
- **Date**: 2026-06-29
- **Refs**: **supersedes the open mechanics (steps 2–3) of**
  [0026](0026-arena-enter-one-sig-genuine-two-party.md) — keeps all of its goals
...
ADR-0026 fixed the arena flow but, in its mechanics (step 2), had the **user's**
...
- `close_cooperative[_with_root]` has **no sender check** — both signatures are
  ...
  because it is the always-online party (already 0026 step 4), not because of its
  seat.
...
tunnel is live. The only freedom is **where the user's wait sits**. ADR-0026 put
...
Everything else from 0026 holds: user = seat A, bot = seat B, bot/settler submits
...
- **Griefing / DoS — the price of putting the house first.** `allocate` now makes
  ...
  5000-CCU target.** ADR-0026's user-creates-first ordering ("1b") is structurally
...
- **`SuiAnchor` scope grows.** The fleet's on-chain seam now **creates** tunnels
  ...
  just deposit-B + settle as 0020/0026 implied.
