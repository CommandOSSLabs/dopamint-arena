# 0029 — Settleable-at-every-state balances (cash-out invariant)
...
Chicken Cross implements it with `settleShare`, in **both** the TypeScript SDK
(`sui-tunnel-ts/src/protocol/cross.ts`) and the Rust port
(`rust/protocols/cross`), computed byte-identically (Rust uses `u128` math to
match the TS bigint floor) so the two stay in wire parity. The lead in
furthest-lane `score` maps linearly to the pot as a fraction of the race distance
(level ⇒ 50/50; a full-distance lead ⇒ winner-take-all). A stalling chicken only
locks its current share, which *erodes* as the opponent advances.
...
- **Easier**: the griefing-draw class is eliminated for progress-gradient games;
  ...
  test in both languages guards the regression; zero on-chain change.
- **Trade-offs**: an abandoned match now settles **proportional to progress
  ...
  invariant test is mandatory, and any change must land in TS **and** Rust
  together or wire parity breaks.
