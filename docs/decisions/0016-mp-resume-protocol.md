...
- **Refs**: implements the deferred "resume protocol" from
  [ADR-0023](0015-data-plane-local-control-plane-redis.md) §Consequences and §Open questions.
...
returning player to re-attach. ADR-0023 deferred the resume protocol to its own ADR. This is it.

The core tension: a correct resume path must not add Redis or on-chain ops to the per-move
relay path (ADR-0023's central invariant), yet it must handle the returning client's need for
