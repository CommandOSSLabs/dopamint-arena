...
- **Refs**: refines [ADR-0005](0005-redis-backed-ha-control-plane.md) (Redis-backed
  HA) and supersedes its *Latency* consequence; premised on
...
that premise no longer holds: the relay *is* the data plane, and a Redis
...
*fallback*). Co-location is a latency optimization that shifts traffic onto the
...
- **Throughput scales by adding instances.** The data plane is shard-local and
  ...
  ~50k cap.
- **Supersedes ADR-0005's *Latency* consequence.** "One Redis round-trip per
