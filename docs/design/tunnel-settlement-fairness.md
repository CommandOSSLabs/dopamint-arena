...
> Decision record: [../decisions/0020-settleable-at-every-state-balances.md](../decisions/0020-settleable-at-every-state-balances.md).
...
state (ADR-0020), not a frozen split. Chicken Cross uses `settleShare`: the lead in
...
- Correctness depends on a monotonic `balances()`; a future game that gets it wrong
  is a new vector, so the settle-fairness invariant test is mandatory.
