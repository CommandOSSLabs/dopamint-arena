...
- **auto/max (mode-aware, tuned during B3 from real data):** `--workers auto` = `round(1.5 × cores)`. `--concurrency auto` is mode-aware: CPU-bound (offchain+local) → low constant (2/worker, since workers already saturate cores); I/O-bound (onchain or relay) → high, memory-capped (70% `os.totalmem()` budget). Explicit `--workers`/`--concurrency` always override. Rationale: workers=cores is true multi-core parallelism; concurrency only overlaps I/O waiting, so high concurrency on CPU-bound work just thrashes (an offchain `--duration 10` ran 79s before this fix).
